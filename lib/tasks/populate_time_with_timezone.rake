namespace :measurements do
  task populate_time_with_timezone: :environment do
    batch_size = 50_000
    sleep_time = 0.5
    total_processed = 0
    ...
    total_to_update = Measurement.where(time_with_timezone: nil).count
    ...
    ActiveRecord::Base.connection.execute("SELECT DISTINCT session_id FROM session_timezones").each do |row|
      ...
      Measurement.where(stream_id: Stream.select(:id).where(session_id: session_id), time_with_timezone: nil).find_in_batches(batch_size: batch_size) do |batch|
        measurement_ids = batch.map(&:id)

        unless measurement_ids.empty?
          sql = <<-SQL
            UPDATE measurements
            SET time_with_timezone = time at time zone '#{timezone_name}'
    ...
    puts "Finished populating time_with_timezone for measurements."
  end
end
