...
class Session < ApplicationRecord
  ...
  # uuids are generated by the clients, which are not consistent about case, and
  # the uniqueness index is on LOWER(uuid) — so an exact match can miss a session
  # that does exist. Index-backed: index_sessions_on_lower_uuid.
  scope :by_uuid, ->(uuid) { where('LOWER(sessions.uuid) = LOWER(?)', uuid.to_s) }
