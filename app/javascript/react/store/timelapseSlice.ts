...
export const selectCurrentTimestamp = (state: RootState) =>
  state.timelapse.currentTimestamp;
export const selectIsLoading = (state: RootState) => state.timelapse.isLoading;
export const selectFilteredTimelapseData = (streamId: string) =>
  createSelector(selectTimelapseData, (timelapseData) => {
    const filteredData: {
      [timestamp: string]: { [streamId: string]: number }[];
    } = {};

    Object.keys(timelapseData).forEach((timestamp) => {
      const sessionsAtTimestamp = timelapseData[timestamp];

      const filteredSessions = sessionsAtTimestamp.filter((session) =>
        Object.keys(session).includes(streamId)
      );

      if (filteredSessions.length > 0) {
        filteredData[timestamp] = filteredSessions;
      }
    });
