...
function createCustomMarkerClass() {
  return class CustomMarker extends google.maps.OverlayView {
    ...
    setNotes(notes: Note[], initialSlide: number = 0) {
      ...
      if (this.div) {
        const noteElements =
          this.div.querySelectorAll("[data-note-container]");
    ...
    static closeAllPopovers(map: google.maps.Map | null) {
      if (map && (map as any).__customMarkers) {
        (map as any).__customMarkers.forEach(
          (marker: InstanceType<typeof CustomMarker>) => {}
        );
      }
    }
...
let _CustomMarkerClass: ReturnType<typeof createCustomMarkerClass> | null =
  null;

export function getCustomMarkerClass() {
  if (!_CustomMarkerClass) {
    _CustomMarkerClass = createCustomMarkerClass();
  }
  return _CustomMarkerClass;
}

export type CustomMarker = InstanceType<
  ReturnType<typeof createCustomMarkerClass>
>;
