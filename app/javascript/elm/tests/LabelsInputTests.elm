...
all =
    describe "view"
        [ fuzz string "label area has a description" <|
            \description ->
                LabelsInput.view LabelsInput.empty description "input-id" "placeholder" False Tooltip.profilesFilter "tooltip-icon.svg"
                    |> Query.fromHtml
                    |> Query.has [ Slc.text description ]
        , fuzz bool "label input can be disabled" <|
            \isDisabled ->
                LabelsInput.view LabelsInput.empty "description" "input-id" "placeholder" isDisabled Tooltip.profilesFilter "tooltip-icon.svg"
                    |> Query.fromHtml
