...
test('distanceBetweenInPixels calculatets the correct distance', t => {
  const latLng1 = { lat: () => 142.16963925333334, lng: () => 86.75634952273033 };
  const latLng2 = { lat: () => 142.16964181333333, lng: () => 86.75637267239966 };
  const fromLatLngToPoint = latLng => ({ x: latLng.lat(), y: latLng.lng() });
  const zoom = 21;

  const actual = distanceBetweenInPixels(latLng1, latLng2, fromLatLngToPoint, zoom);
