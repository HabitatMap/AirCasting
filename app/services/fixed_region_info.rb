class FixedRegionInfo
  ...
  def last_measurement_time(streams_ids)
    Measurement.with_streams(streams_ids).select('MAX(time) as last_time').first
      .last_time
  end

  def calculate_average(streams_ids, end_time)
    streams_ids.map { |stream_id| one_hour_average(stream_id, end_time) }.sum /
      streams_ids.size
  end
  ...
  def number_of_samples(streams_ids, end_time)
    Measurement.with_streams(streams_ids).where(
      time: (end_time - 1.hour)..end_time
    )
      .count
  end
