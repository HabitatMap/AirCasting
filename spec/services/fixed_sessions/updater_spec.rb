...
RSpec.describe FixedSessions::Updater do
  subject(:updater) { described_class.new }

  let(:user) { create(:user) }
  let(:session) { create(:fixed_session, user: user, title: 'Old title', tag_list: 'a, b', version: 3) }
  ...
  it 'leaves is_indoor, contribute, coordinates and device alone' do
    updater.call(session: session, data: { title: 'Renamed' })
    session.reload
    expect(session.is_indoor).to eq(false)
    expect(session.contribute).to eq(true)
  end
