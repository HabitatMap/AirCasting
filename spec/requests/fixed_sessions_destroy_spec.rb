...
describe 'DELETE /api/v3/fixed_sessions/:uuid' do
  include Devise::Test::IntegrationHelpers

  let(:user) { create(:user) }

  def delete_session(uuid, headers = {})
    delete "/api/v3/fixed_sessions/#{uuid}",
           headers: {
  ...
  it 'answers 204 on a retry that differs only in uuid case' do
    session = create(:fixed_session, user: user, uuid: SecureRandom.uuid)
    delete_session(session.uuid)
  ...
  it "does not let one user's tombstone answer for another" do
    ...
    other_session = create(:fixed_session, user: other_user, uuid: SecureRandom.uuid)
  ...
  it 'refuses a mobile session through the fixed route' do
    ...
    expect { delete_session(mobile.uuid) }.not_to change(Session, :count)
  ...
  # session management — the device must not be able to delete the session.
  it 'rejects the session_token that authenticates uploads' do
    sign_out user
    session = create(:fixed_session, user: user, session_token: SecureRandom.hex(16))

    expect { delete_session(session.uuid, bearer(session.session_token)) }
  ...
  describe 'when the delete fails' do
    ...
    it 'answers internal_error in the v3 error shape' do
      allow_any_instance_of(FixedSessions::Destroyer)
        .to receive(:call)
        .and_return(
