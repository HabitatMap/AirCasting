...
describe User do
  ...
  describe "#sync" do
    let(:session1) { FactoryGirl.create(:timeboxed_session, :user => user) }
    let(:session2) { FactoryGirl.create(:timeboxed_session, :user => user, :notes => [note1, note2]) }
    let!(:session3) { FactoryGirl.create(:timeboxed_session, :user => user) }
    let(:session4) { FactoryGirl.create(:timeboxed_session, :user => user, :notes => [note3]) }
    let(:session5) { FactoryGirl.create(:timeboxed_session, :user => user) }
