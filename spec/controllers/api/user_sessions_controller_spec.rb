...
describe Api::UserSessionsController do
  ...
  describe "#show" do
    ...
    context "getting own session" do
      let(:session) { FactoryGirl.create(:timeboxed_session, :user => user, :streams => [stream], :tag_list => "hello world") }
    ...
    context "session has notes with photos" do
      ...
      let(:session) { FactoryGirl.create(:timeboxed_session, :user => user, :notes => [note]) }
