...
require 'spec_helper'
require './lib/timeboxed_session_builder'

describe TimeboxedSessionBuilder do
  ...
  subject { TimeboxedSessionBuilder.new(session_data, photos, user) }
  ...
  describe "#build_session!" do
    it "should process the data" do
      TimeboxedSessionBuilder.should_receive(:prepare_notes).with(:note_data, photos).and_return(:prepared_notes)
      TimeboxedSessionBuilder.should_receive(:normalize_tags).with(:denormalized_tags).and_return(:normalized_tags)
      data = {
        :some => :data,
  ...
  describe ".prepare_notes" do
    it "should match the photos" do
      TimeboxedSessionBuilder.prepare_notes([{:note => :one}, {:note => :two}], [:photo1, :photo2]).
  ...
  describe '.normalize_tags' do
    it 'should replace spaces and commas with commas as tag delimiters' do
      TimeboxedSessionBuilder.normalize_tags('jola misio, foo').should == 'jola,misio,foo'
    end
  end
